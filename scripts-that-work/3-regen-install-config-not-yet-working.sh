...
# 3) Reboot.
# 4) Run this script to install regen-ledger and configure it.
# sentry example:
# ./3-regen-install-config.sh denom chain-name myname my-private-ip my-public-ip "seed-node" "persistent-peers" "private-peers"
# ./3-regen-install-config.sh "v1.0.0" uregen regen-1 my-sentry-357 192.168.1.2 11.12.13.14 "seed" "persistent-p" "privp"
# validator example
# ./3-regen-install-config.sh "v1.0.0" uregen regen-1 my-validator-357 192.168.1.2 "" "" "persistent-p" ""

# Set versions of cosmos-sdk (for cosmovisor) and regen-ledger before running this script.

# sentry node variables
DAEMON=regen
REGENVERSION=$1
DENOM=$2
CHAIN_ID=$3
...
regen init --chain-id $CHAIN_ID $YOUR_NAME
...
curl https://raw.githubusercontent.com/regen-network/mainnet/main/regen-1/checksum.txt

echo "The checksums above should match. If not, you have the wrong version of genesis.json."
read -s -d ' '

# reuse this code to insert the config data in the correct places
